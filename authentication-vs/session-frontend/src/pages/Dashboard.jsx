        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between border-b border-slate-800 pb-5 gap-4">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">
              Active Session Node: <span className="font-mono text-cyan-400">Stateful HttpSession</span>
            </p>
          </div>
          <div>
            <button
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 backdrop-blur space-y-6">
            ...
            <div className="space-y-4 font-mono text-sm">
              ...
              <div className="flex justify-between border-b border-slate-800/60 pb-2">
                ...
                <span className="text-cyan-400">Session Cookie (JSESSIONID)</span>
              </div>
              <div className="flex justify-between border-b border-slate-800/60 pb-2">
            ...
            <div className="border-t border-slate-800 pt-6 space-y-4">
              <h3 className="text-sm font-semibold text-slate-300">Test API Calls</h3>
              <div className="flex flex-wrap gap-3">
                <button
                  onClick={() => handleSimulate('/session/profile')}
                  disabled={simLoading}
            ...
            <form onSubmit={handleChangePassword} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="block w-full rounded-lg border border-slate-700 bg-slate-950/50 px-3 py-2 text-sm text-slate-200 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                  placeholder="••••••••"
                />
              </div>
              <div>
              ...
              <button
                type="submit"
                disabled={pwLoading}
                className="w-full rounded-lg bg-indigo-650 px-4 py-2.5 text-xs font-semibold text-white shadow-lg hover:brightness-110 disabled:opacity-50 transition"
              >
                {pwLoading ? 'Processing...' : 'Apply Password Change'}
              </button>
              <p className="text-[10px] text-slate-500 italic mt-2">
                Note: Stateful systems immediately invalidate the active HTTP Session upon credential changes, prompting redirect.
              </p>
            </form>
          </div>
        </div>
        ...
        {user?.role === 'ROLE_ADMIN' && (
          <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-6 backdrop-blur">
                ...
                <tbody className="divide-y divide-slate-800 bg-slate-900/20">
                  {adminUsers.map((u) => (
                    <tr key={u.id} className="hover:bg-slate-800/10">
                      ...
                      <td className="px-6 py-4 text-indigo-400">{u.role}</td>
            ...
            <p className="text-[10px] text-slate-500 italic mt-3">
              Note: If a user is blocked in a stateful setup, their session is invalidated immediately on their next request.
            </p>
          </div>
        )}
